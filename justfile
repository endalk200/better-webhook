...
cli-format-check:
    devbox run -- bash -lc 'files="$(cd {{ invocation_directory() }}/apps/cli && gofmt -l ./cmd ./internal ./main.go)"; if [ -n "$files" ]; then printf "%s\n" "$files"; exit 1; fi'

# Lint the standalone Go CLI.
cli-lint:
    devbox run -- go -C {{ invocation_directory() }}/apps/cli vet ./...

# Test the standalone Go CLI.
cli-test:
    devbox run -- go -C {{ invocation_directory() }}/apps/cli test ./...

# Build the standalone Go CLI with explicit release metadata.
cli-build version="dev" commit="unknown" date="unknown":
    devbox run -- mkdir -p {{ invocation_directory() }}/apps/cli/bin
    CGO_ENABLED=0 devbox run -- go -C {{ invocation_directory() }}/apps/cli build -trimpath -ldflags "-s -w -X github.com/endalk200/better-webhook/apps/cli/internal/version.Version={{ version }} -X github.com/endalk200/better-webhook/apps/cli/internal/version.Commit={{ commit }} -X github.com/endalk200/better-webhook/apps/cli/internal/version.Date={{ date }}" -o {{ invocation_directory() }}/apps/cli/bin/bw .
