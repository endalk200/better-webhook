...
19. As an application developer, I want in-progress idempotency reservations to be distinct from completed duplicates, so that concurrent deliveries do not get acknowledged as successful before processing finishes.
20. As an application developer, I want **Replay Protection** to be separate from **Idempotency**, so that stale or repeated signed delivery attempts are treated as security events rather than duplicate business events.
21. As an application developer, I want provider timestamp tolerance enforced by default when the provider supports signed timestamps, so that stale signed requests are rejected without requiring a store.
22. As an application developer, I want optional seen-delivery tracking through a **Replay Store**, so that I can strengthen replay protection when I have shared storage.
23. As an application developer, I want memory replay support for tests and local development, so that replay behavior can be exercised without external infrastructure.
24. As an application developer, I want the handler context to contain only the **Webhook Event**, **Webhook Delivery**, and abort signal, so that handlers remain framework-neutral.
25. As an application developer, I want handler return values ignored, so that HTTP response semantics are controlled consistently by the pipeline instead of individual handlers.
26. As a Next.js developer, I want a Next.js adapter, so that I can expose a Better Webhook endpoint from route handlers without rewriting the pipeline.
27. As an Express developer, I want an Express adapter, so that I can mount a Better Webhook endpoint while preserving raw body bytes.
28. As a framework user, I want adapters to avoid provider verification logic, so that all frameworks share the same security behavior.
29. As an observability-focused developer, I want OpenTelemetry support, so that webhook processing appears in distributed traces and metrics.
30. As a security-conscious developer, I want telemetry to avoid payload contents by default, so that sensitive webhook data is not accidentally exported.
31. As an operator, I want delivery spans and attributes for provider, event type, verification result, replay result, idempotency result, handler result, and response status, so that I can diagnose webhook behavior.
32. As a TypeScript developer, I want Stripe event-specific payload typing for a curated initial set of common events, so that handler code has useful types without claiming total Stripe coverage.
33. As a TypeScript developer, I want unknown Stripe event types to remain verified and catch-all handleable, so that the SDK remains useful when Stripe sends events outside the curated map.
34. As a maintainer, I want provider packages to validate **Event Envelopes** at runtime by default, so that required metadata extraction is reliable.
35. As a maintainer, I want full provider payload runtime validation deferred or optional, so that incomplete schemas do not reject valid provider deliveries.
36. As a maintainer, I want Stripe verification implemented without depending on the official Stripe package initially, so that dependency weight and release coupling stay low.
37. As a maintainer, I want provider compatibility tests using raw bytes and real signature semantics, so that provider packages prove behavior beyond simple happy-path unit tests.
38. As a maintainer, I want core to expose lower-level primitives where appropriate, so that advanced users and adapters can test or compose focused behavior without bypassing the supported endpoint API.
39. As a maintainer, I want framework adapters to declare raw body and raw header preservation limits, so that users understand framework-specific signature verification constraints.
40. As a maintainer, I want the pipeline to expose narrow extension points rather than general middleware, so that users cannot accidentally reorder security-critical steps.
41. As a package consumer, I want the SDK to target Node.js 18+ and ESM, so that the initial package contract is modern and testable.
42. As a package consumer, I want TypeScript declarations published with every SDK package, so that the SDK is usable in typed applications.
43. As a future contributor, I want package boundaries documented, so that provider logic does not drift into adapters or core-specific provider switches.
44. As a future contributor, I want runtime and module-format constraints documented, so that Edge, browser, and CommonJS support are not accidentally assumed.
...
- Core uses a small **Raw Delivery Request** abstraction that preserves method, URL, raw headers, duplicate header values when available, abort signal, and unmodified raw body bytes. Fetch-compatible conveniences may exist, but framework objects are not the core contract.
...
- Framework adapters must declare which raw body and raw header capabilities their framework can preserve. If a framework cannot expose duplicate raw header lines, the adapter must document that limitation and tests must verify the best available representation.
...
- Default response semantics are: handled events return `200`, ignored events return `200`, duplicate completed events return `200`, in-progress idempotency reservations return `409`, invalid signatures return `400`, replay rejects return `400`, unsupported invalid input returns `400`, and handler errors return `500`.
...
- A stable developer-configured **Endpoint Identity** is required when an **Idempotency Store** is configured.
- Core provides memory idempotency support for tests and local development.
- The **Idempotency Store** contract must support atomic reservation before handling, an in-progress reservation outcome distinct from completed duplicates, completion after successful handling, and failure handling that releases the reservation by default.
- In-progress idempotency reservations must not be acknowledged as completed duplicates. The default response for a concurrent in-progress reservation is `409` so the provider can retry later.
...
- Replay store keys are provider-derived delivery replay keys that include provider identity, **Endpoint Identity**, and provider-specific signed delivery material. For Stripe, the delivery replay key is derived from the Stripe timestamp, the verified Stripe signature value, and a digest of the exact raw body bytes.
...
- Core pipeline tests must cover successful handling, ignored events, catch-all handling, handler failure, response policy mapping, raw-body preservation, rejected verification, replay rejection, idempotency duplicate behavior, in-progress idempotency behavior, and idempotency release-on-failure.
- Idempotency coordinator tests must cover atomic reservation semantics, duplicate completed events, in-progress reservation responses, release-on-failure, completion-after-success, required endpoint identity, TTL behavior where applicable, and concurrent delivery scenarios where practical.
- Replay protection tests must cover timestamp tolerance acceptance and rejection, provider-derived replay key generation, optional seen-delivery tracking, no-store behavior, and separation from idempotency.
...
- Next.js adapter tests must verify request conversion, raw body preservation, declared raw header capability limits, response translation, rejected delivery propagation, and successful handler invocation through the adapter.
- Express adapter tests must verify raw body handling expectations, duplicate/raw header preservation where available, declared raw header capability limits, request conversion, response translation, rejected delivery propagation, successful handler invocation, and failure response behavior.
...
- Documentation should explain the raw request contract, including why adapters must preserve raw body bytes and signature-relevant headers before JSON parsing or other transformations, and where framework adapters have header preservation limits.
