...
- When using `devbox run` through Codex shell commands, the command may still execute from the repo root even if the tool `workdir` points at a subdirectory. For subdirectory-specific Go commands, prefer `devbox run -- zsh -lc 'cd <dir> && ...'`.
- GitHub Actions runners used with `devbox run` do not reliably have `zsh`; checked-in automation such as `justfile` recipes and workflow-invoked shell commands should use `bash` or POSIX `sh` instead of `zsh`. Keep `zsh -lc` as a Codex-only workaround for ad hoc local shell commands when `workdir` handling is the real issue.
