...
export function toRawDeliveryRequest(request: ExpressWebhookRequest): RawDeliveryRequest {
...
export function createExpressMiddleware(endpoint: WebhookEndpoint) {
	return async (
		...
		try {
			sendExpressResponse(response, await endpoint.handle(toRawDeliveryRequest(request)));
		} catch (error) {
			next(error);
		}
	};
...
function rawHeadersToPairs(request: ExpressWebhookRequest): { name: string; value: string }[] {
