...
export function github(
  ...
  return {
    ...
    verify(delivery) {
      ...
      if (!signature) return { ok: false, reason: "missing_github_signature" };
      ...
      const expected = computeGitHubSignature(
...
export function parseGitHubSignatureHeader(header: string): string | undefined {
  if (!header.startsWith("sha256=")) return undefined;
...
export function parseGitHubEnvelope(
  ...
  if (!headers.delivery) throw new Error("GitHub Delivery ID is required");
  if (!headers.event) throw new Error("GitHub event name is required");
  ...
  if (!isRecord(parsed)) throw new Error("GitHub payload must be an object");
...
export function isKnownGitHubEventType(
  type: string,
): type is KnownGitHubEventType {
