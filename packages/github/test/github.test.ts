  ...
  return {
    ...
    headers: [
      ...
      { name: "x-github-event", value: options.event ?? "pull_request" },
      {
        ...
        value:
          options.signature ?? createGitHubSignatureHeader({ secret, rawBody }),
      },
...
describe("github provider", () => {
  ...
  it("rejects missing, malformed, mismatched, and mutated signatures", async () => {
    ...
    expect(
      await provider.verify({
        ...delivery,
        headers: delivery.headers.filter(
          (header) => header.name !== "x-hub-signature-256",
        ),
      }),
  ...
  it("accepts JSON and rejects form-encoded deliveries", async () => {
    ...
    expect(
      await provider.verify({
        method: "POST",
        url: "https://example.test/github",
        headers: request({ contentType: "application/json; charset=utf-8" })
          .headers,
        rawBody: new TextEncoder().encode(body()),
      }),
  ...
  it("validates the event envelope only", async () => {
    ...
    expect(
      (await endpoint.handleWithResult(request({ deliveryId: "" }))).result,
    ).toMatchObject({
      status: "unsupported",
      reason: "invalid_event_envelope",
    });
    expect(
      (await endpoint.handleWithResult(request({ event: "" }))).result,
    ).toMatchObject({
      status: "unsupported",
      reason: "invalid_event_envelope",
    });
  ...
  it("classifies known events without inspecting action values", async () => {
    ...
    const endpoint = createWebhookEndpoint({
      ...
      handlers: Object.fromEntries(
        knownGitHubEventTypes.map((type) => [type, handler]),
      ),
    });
  ...
  it("keeps unknown events catch-all handleable", async () => {
    ...
    const result = await endpoint.handleWithResult(
      request({ event: "repository_ruleset" }),
    );
  ...
  it("uses GitHub Delivery ID for idempotency and replay keys", async () => {
    ...
    expect(
      (await replayEndpoint.handleWithResult(request())).result.status,
    ).toBe("handled");
    expect(
      (await replayEndpoint.handleWithResult(request())).result,
    ).toMatchObject({ status: "rejected", reason: "replayed_delivery" });
  });
  ...
  it("narrows known handlers and exposes unknown fallback types", () => {
    createWebhookEndpoint({
      ...
      handlers: {
        pull_request: ({ event }) => {
          expectTypeOf(event.payload.pull_request?.title).toEqualTypeOf<
            string | undefined
          >();
        },
        "*": ({ event }) => {
    ...
    expectTypeOf<
      Extract<GitHubWebhookEvent, UnknownGitHubEvent>["payload"]
