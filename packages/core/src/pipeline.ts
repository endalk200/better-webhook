...
export type RawDeliveryRequest = {
	...
	body: Uint8Array | ArrayBuffer | string | (() => Promise<Uint8Array | ArrayBuffer | string>);
	signal?: AbortSignal;
};
...
export type IdempotencyStore = {
	reserve(key: string, ttlMs?: number): Promise<IdempotencyReservation> | IdempotencyReservation;
	complete(key: string, ttlMs?: number): Promise<void> | void;
	fail(key: string): Promise<void> | void;
};

export type ReplayStore = {
	remember(key: string, ttlMs: number): Promise<"stored" | "seen"> | "stored" | "seen";
};
...
export type DeliveryTelemetry = {
	startDelivery?(delivery: TelemetryDeliveryStart): DeliveryTelemetryContext | undefined;
	finishDelivery?(
		delivery: TelemetryDeliveryEnd,
		context: DeliveryTelemetryContext | undefined,
	): void;
	recordError?(
		error: {
			provider: string;
...
export function createWebhookEndpoint<TEvents extends WebhookEvent>(
	...
	if (options.idempotencyStore && !options.endpointIdentity) {
		throw new Error("endpointIdentity is required when idempotencyStore is configured");
	}
	if (options.replayStore && !options.endpointIdentity) {
		throw new Error("endpointIdentity is required when replayStore is configured");
	}

	const responsePolicy = options.responsePolicy ?? defaultResponsePolicy;

	const handleWithResult: WebhookEndpoint["handleWithResult"] = async (request) => {
...
export async function toWebhookDelivery(request: RawDeliveryRequest): Promise<WebhookDelivery> {
	const value = typeof request.body === "function" ? await request.body() : request.body;
	return {
...
export function getHeaderValues(headers: RawHeaderValue[], name: string): string[] {
...
export function defaultResponsePolicy(result: PipelineResult): WebhookResponse {
	...
	return {
		...
		body: JSON.stringify({
			ok:
				result.status === "handled" || result.status === "ignored" || result.status === "duplicate",
		}),
	};
}
...
export function createMemoryIdempotencyStore(): IdempotencyStore & {
	...
	const entries = new Map<string, { status: "in_progress" | "completed"; expiresAt?: number }>();
...
function checkReplayTimestamp<TEvents extends WebhookEvent>(
	...
	if (verification.signedTimestamp) {
		const ageMs = Math.abs(now.getTime() - verification.signedTimestamp.getTime());
		if (ageMs > windowMs) {
			return { status: "rejected", reason: "stale_signed_timestamp" };
		}
	}

	return verification.signedTimestamp ? { status: "accepted" } : { status: "not_configured" };
}
