  ...
  .action(
    async (
      ...
      try {
        ...
        console.log(`🔄 Run it with: better-webhook webhooks run ${templateName}`);
...
export const capture = new Command()
  .name("capture")
  .description("Start a server to capture incoming webhook requests, or list captured webhooks")
