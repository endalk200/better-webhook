# Next.js + Stripe Example Application

This Example Application is a Workspace Consumer for one Next.js Framework Adapter and one Stripe Provider Definition. It is a local Capability Showcase for signature verification, Idempotency, Replay Protection, Event Handlers, response behavior, console Manual Example Feedback, and Delivery Observability.

It uses an App Router Route Handler with the Node.js runtime and keeps only the minimum Next.js surface needed to run the Webhook Endpoint. It is self-contained under this directory and intentionally does not share helper modules with other examples.
...
```sh
devbox run -- pnpm --filter @better-webhook/example-nextjs-stripe run dev
```

The default Webhook Endpoint is `http://127.0.0.1:3001/api/webhooks/stripe`.
...
```sh
devbox run -- pnpm --filter @better-webhook/example-nextjs-stripe run send:checkout
devbox run -- pnpm --filter @better-webhook/example-nextjs-stripe run send:invoice
devbox run -- pnpm --filter @better-webhook/example-nextjs-stripe run send:unknown
devbox run -- pnpm --filter @better-webhook/example-nextjs-stripe run send:duplicate
devbox run -- pnpm --filter @better-webhook/example-nextjs-stripe run send:replay
devbox run -- pnpm --filter @better-webhook/example-nextjs-stripe run send:ignored
devbox run -- pnpm --filter @better-webhook/example-nextjs-stripe run send:failure
```
...
- `OTEL_SERVICE_NAME`: service name, default `better-webhook-example-nextjs-stripe`.
