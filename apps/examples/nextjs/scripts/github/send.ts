...
if (!scenario) {
  console.error(
    "Usage: pnpm run send:github:<pull-request|issue-comment|check-run|duplicate|replay|ignored|unknown|failure>",
  );
  process.exit(1);
}
...
async function postDelivery(
  ...
  console.log(
    `[sender:nextjs:github] ${label} status=${response.status} body=${await response.text()}`,
  );
}
