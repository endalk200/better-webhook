...
export const stripeConfig = {
	...
	endpointIdentity: "example-express-stripe-local",
	...
	signingSecret: process.env.STRIPE_WEBHOOK_SECRET ?? "whsec_better_webhook_local_example",
	webhookPath: "/webhooks/stripe",
};

export function stripeWebhookUrl(): string {
	return (
		process.env.STRIPE_WEBHOOK_URL ?? `http://127.0.0.1:${config.port}${stripeConfig.webhookPath}`
	);
}
