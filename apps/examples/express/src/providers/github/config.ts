...
export const githubConfig = {
	...
	endpointIdentity: "example-express-github-local",
	...
	webhookSecret: process.env.GITHUB_WEBHOOK_SECRET ?? "github_better_webhook_local_example",
};

export function githubWebhookUrl(): string {
	return (
		process.env.GITHUB_WEBHOOK_URL ?? `http://127.0.0.1:${config.port}${githubConfig.webhookPath}`
	);
}
