...
| `onSuccess` | `(eventType: string) => void \| Promise<void>` | Called after a registered handler successfully processes the webhook. |
...
1. Adapter options
2. Provider options
3. Environment variables (`GITHUB_WEBHOOK_SECRET`, `STRIPE_WEBHOOK_SECRET`, `RAGIE_WEBHOOK_SECRET`, `RECALL_WEBHOOK_SECRET`, `RESEND_WEBHOOK_SECRET`, or `WEBHOOK_SECRET`)
