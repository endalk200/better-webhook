...
| `-v, --verbose`       | `false`       | Enable verbose capture logging (enabled by default when `log_level=debug` unless overridden) |
...
| `-v, --verbose`           | Show detailed request/response info (enabled by default when `log_level=debug` unless overridden) |
| `--captures-dir <dir>`    | Directory where captures are stored         |
