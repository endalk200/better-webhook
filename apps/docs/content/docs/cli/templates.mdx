...
If a single template is already present, running `download <template-id>` again reports that it is already downloaded. Use `--refresh` to force a re-download.
...
<Callout type="info">
  ...
  1. `<PROVIDER>_WEBHOOK_SECRET` (for example `GITHUB_WEBHOOK_SECRET` or `RAGIE_WEBHOOK_SECRET`)
  2. `WEBHOOK_SECRET`
</Callout>
