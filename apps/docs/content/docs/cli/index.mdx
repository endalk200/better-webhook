...
<Tabs groupId="package-manager" items={["npm", "pnpm", "yarn"]}>
  <Tab value="npm">```bash npm install -g @better-webhook/cli ```</Tab>
  <Tab value="pnpm">```bash pnpm add -g @better-webhook/cli ```</Tab>
  <Tab value="yarn">```bash yarn global add @better-webhook/cli ```</Tab>
</Tabs>
